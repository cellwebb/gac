...
**참고:** `--group`을 사용할 때, 최대 출력 토큰 제한은 커밋되는 파일 수에 따라 자동으로 조정됩니다 (1-9개 파일에 2 배, 10-19개 파일에 3 배, 20-29개 파일에 4 배, 30개 이상 파일에 5 배). 이는 LLM이 대량의 변경셋에도 잘림 없이 모든 그룹화된 커밋을 생성할 수 있도록 충분한 토큰을 확보합니다.
