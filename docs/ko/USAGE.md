...
- `GAC_DISABLE_STATS=true` - 사용 통계 추적 비활성화 (통계 파일 읽기/쓰기를 하지 않음; 기존 데이터는 보존됨)
...
- `gac stats show` — gac 사용 통계 보기 (총계, 연속 사용, 일일 및 주간 활동, 상위 프로젝트)
...
gac은 가벼운 사용 통계를 추적하여 커밋 활동, 연속 사용 일수, 가장 활발한 프로젝트를 확인할 수 있게 합니다. 통계는 `~/.gac_stats.json`에 로컬로 저장되며 어디에도 전송되지 않습니다.

**추적하는 내용:** gac 실행 총횟수, 커밋 총횟수, 최초/최근 사용 날짜, 일일 및 주간 카운트, 현재 및 최장 연속 일수, 프로젝트별 활동 카운트.

**추적하지 않는 내용:** 커밋 메시지, 코드 내용, 파일 경로, 개인 정보 또는 카운트와 프로젝트 이름을 넘어서는 데이터.
...
| `gac stats show`    | 전체 통계 표시: 총계, 연속 사용, 일일 및 주간 활동, 상위 프로젝트 |
...
- **활동 요약** — 오늘 및 이번 주 gac/커밋 수와 피크 일 및 피크 주 비교
- **상위 프로젝트** — gac 사용 + 커밋 수 기준 상위 5개 가장 활발한 저장소
- **하이 스코어 축하** — 🏆 새로운 일일, 주간 또는 연속 기록을 세우면 트로피 획득; 🥈 기록과 타이면 획득
- **격려 메시지** — 활동에 기반한 맥락적 응원

### 통계 비활성화

`GAC_DISABLE_STATS` 환경 변수를 임의의 값으로 설정하세요:
...
비활성화하면 gac은 모든 통계 기록을 건너뜁니다 — 파일 읽기나 쓰기가 발생하지 않습니다. 기존 데이터는 보존되지만 변수 설정을 해제할 때까지 업데이트되지 않습니다.
