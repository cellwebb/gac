...
`gac init`를 실행할 때, 프로바이더로 "Claude Code"를 선택하기만 하면 됩니다:
...
브라우저가 자동으로 열려 새로운 OAuth 인증이 진행됩니다. 또는 `gac model`을 실행하고 "Claude Code (OAuth)"를 선택한 다음 "재인증(새 토큰 받기)"를 선택할 수도 있습니다.
