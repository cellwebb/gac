...
- `GAC_DISABLE_STATS=true` - 使用統計の追跡を無効化（統計ファイルの読み書きを行わない；既存のデータは保持されます）
...
- `gac stats show` — gac の使用統計を表示（合計、ストリーク、日次・週次アクティビティ、トッププロジェクト）
- `gac stats project` — 現在の git プロジェクトのみの統計を表示
...
gac は軽量な使用統計を追跡し、コミットアクティビティ、ストリーク、最もアクティブなプロジェクトを確認できます。統計は `~/.gac_stats.json` にローカルで保存され、どこにも送信されません。

**追跡内容:** gac 実行回数の合計、コミット回数の合計、初回/最終使用日、日次および週次カウント、現在および最長ストリーク、プロジェクト別アクティビティカウント。

**追跡しない内容:** コミットメッセージ、コード内容、ファイルパス、個人情報、またはカウントとプロジェクト名を超えるデータ。
...
| `gac stats show`    | 完全な統計を表示：合計、ストリーク、日次・週次アクティビティ、トッププロジェクト |
| `gac stats project` | 現在の git プロジェクトのみの統計を表示                                          |
| `gac stats reset`   | すべての統計をゼロにリセット（確認プロンプト付き）                               |
...
- **アクティビティサマリー** — 今日および今週の gac/コミット数とピーク日およびピーク週の比較
- **トッププロジェクト** — gac 使用 + コミット数による上位5つの最もアクティブなリポジトリ
- **ハイスコアのお祝い** — 🏆 新しい日次、週次、またはストリーク記録を樹立したときにトロフィーを獲得；🥈 記録に並んだときに獲得
- **励ましのメッセージ** — アクティビティに基づいたコンテキストに応じた励まし

### 統計の無効化

`GAC_DISABLE_STATS` 環境変数を任意の値に設定します：
...
無効にすると、gac はすべての統計記録をスキップします — ファイルの読み書きは行われません。既存のデータは保持されますが、変数の設定を解除するまで更新されません。
