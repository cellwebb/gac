...
- `GAC_DISABLE_STATS=true` - 禁用使用统计追踪（不读取或写入统计文件；现有数据将保留）
...
- `gac stats show` — 查看你的 gac 使用统计（总计、连续使用、每日和每周活动、热门项目）
- `gac stats project` — 仅查看当前 git 项目的统计
...
gac 追踪轻量级使用统计，让你可以查看提交活动、连续使用天数和最活跃的项目。统计数据本地存储在 `~/.gac_stats.json` 中，不会发送到任何地方。

**追踪内容：** gac 运行总次数、提交总次数、首次/末次使用日期、每日和每周计数、当前和最长连续使用天数，以及每个项目的活动计数。

**不追踪的内容：** 提交信息、代码内容、文件路径、个人信息，或任何超出计数和项目名称的数据。
...
| `gac stats show`    | 显示完整统计：总计、连续使用、每日和每周活动、热门项目 |
| `gac stats project` | 仅显示当前 git 项目的统计                              |
| `gac stats reset`   | 重置所有统计数据为零（需要确认）                       |
...
- **活动摘要** — 今日和本周的 gac 使用/提交数与你的峰值日和峰值周对比
- **热门项目** — 按 gac 使用 + 提交数排列的 5 个最活跃仓库
- **高分庆祝** — 🏆 在你创造新的每日、每周或连续使用记录时获得奖杯；🥈 在追平记录时获得
- **鼓励信息** — 基于你活动的上下文鼓励

### 禁用统计

将 `GAC_DISABLE_STATS` 环境变量设置为任意值：
...
禁用后，gac 将跳过所有统计记录 — 不会读取或写入文件。现有数据将保留，但在取消设置该变量之前不会更新。
