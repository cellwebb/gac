...
Ваш браузер автоматически откроется для новой аутентификации OAuth. Кроме того, вы можете выполнить `gac model`, выбрать "Claude Code (OAuth)" и выбрать "Повторная аутентификация (получить новый токен)".
