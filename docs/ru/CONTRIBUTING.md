...
[English](../en/CONTRIBUTING.md) | [简体中文](../zh-CN/CONTRIBUTING.md) | [繁體中文](../zh-TW/CONTRIBUTING.md) | [日本語](../ja/CONTRIBUTING.md) | [한국어](../ko/CONTRIBUTING.md) | [हिन्दी](../hi/CONTRIBUTING.md) | [Tiếng Việt](../vi/CONTRIBUTING.md) | [Français](../fr/CONTRIBUTING.md) | **Русский** | [Español](../es/CONTRIBUTING.md) | [Português](../pt/CONTRIBUTING.md) | [Norsk](../no/CONTRIBUTING.md) | [Svenska](../sv/CONTRIBUTING.md) | [Deutsch](../de/CONTRIBUTING.md) | [Nederlands](../nl/CONTRIBUTING.md) | [Italiano](../it/CONTRIBUTING.md)
...
- [ ] **2. Зарегистрировать поставщика в пакете** (`src/gac/providers/__init__.py`)
  ...
  - Добавьте в словарь `PROVIDER_REGISTRY`: `"provider-name": call_<provider>_api`
  - Добавьте в список `__all__`: `"call_<provider>_api"`

- [ ] **3. Обновить пример конфигурации** (`.gac.env.example`)
...
- [ ] **4. Обновить документацию** (`README.md` и все переводы `README.md` в `docs/`)

  - Добавьте имя поставщика в секцию "Поддерживаемые поставщики" во всех переводах README
...
- [ ] **5. Добавить в интерактивную настройку** (`src/gac/init_cli.py`)
  ...
  - **Важно**: Если ваш поставщик использует нестандартное имя API-ключа (не автоматически сформированное `{PROVIDER_UPPERCASE}_API_KEY`), добавьте специальную обработку:
    ...
    Примеры: `kimi-for-coding` использует `KIMI_CODING_API_KEY`, `moonshot-ai` использует `MOONSHOT_API_KEY`

- [ ] **6. Создать всеобъемлющие тесты** (`tests/providers/test_<provider>.py`)
...
- [ ] **7. Увеличить версию** (`src/gac/__version__.py`)
...
6. **Регистрация поставщика**: Изменяйте только `src/gac/providers/__init__.py` и `src/gac/init_cli.py` – `ai.py` и `ai_utils.py` автоматически читают из реестра
7. **Формат имени поставщика**: Используйте нижний регистр с дефисами для многословных имен (например, "lm-studio")
8. **Увеличение версии**: Добавление поставщика требует **minor** увеличения версии (новая функция)
