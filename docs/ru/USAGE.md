...
- `GAC_DISABLE_STATS=true` - Отключить отслеживание статистики использования (нет чтения или записи файла статистики; существующие данные сохраняются)
...
- `gac stats show` — Просмотреть статистику использования gac (итоги, серии, ежедневная и еженедельная активность, лучшие проекты)
- `gac stats project` — Просмотреть статистику только для текущего git-проекта
...
gac отслеживает лёгкую статистику использования, чтобы вы могли видеть свою активность коммитов, серии и наиболее активные проекты. Статистика хранится локально в `~/.gac_stats.json` и никогда никуда не отправляется.

**Что отслеживается:** общее количество запусков gac, общее количество коммитов, даты первого и последнего использования, ежедневные и еженедельные счётчики, текущая и самая длинная серия, а также счётчики активности по проектам.

**Что НЕ отслеживается:** сообщения коммитов, содержимое кода, пути к файлам, личная информация или что-либо помимо счётчиков и имён проектов.
...
| `gac stats show`    | Отобразить полную статистику: итоги, серии, ежедневную и еженедельную активность, лучшие проекты |
| `gac stats project` | Показать статистику только для текущего git-проекта                                              |
| `gac stats reset`   | Сбросить всю статистику до нуля (запрашивает подтверждение)                                      |
...
- **Сводка активности** — gac-запуски/коммиты за сегодня и эту неделю по сравнению с вашим пиковым днём и пиковый неделей
- **Лучшие проекты** — 5 ваших наиболее активных репозиториев по количеству gac + коммитов
- **Празднование рекордов** — 🏆 трофеи при установке новых ежедневных, еженедельных рекордов или рекордов серии; 🥈 за их повторение
- **Подбадривающие сообщения** — контекстуальные напоминания на основе вашей активности

### Отключение статистики

Установите переменную окружения `GAC_DISABLE_STATS` в любое значение:
...
При отключении gac пропускает всю запись статистики — не происходит чтения или записи файлов. Существующие данные сохраняются, но не будут обновляться, пока вы не снимете эту переменную.
