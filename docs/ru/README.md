...
- **Простое отключение**: Установите `GAC_DISABLE_STATS=1`, чтобы отключить отслеживание — данные не записываются
- **Конфиденциальность прежде всего**: Хранятся только счётчики, даты и имена проектов (полученные из git remote/каталога) — нет сообщений коммитов, кода или личных данных
