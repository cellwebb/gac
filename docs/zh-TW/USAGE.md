...
- `GAC_DISABLE_STATS=true` - 停用使用統計追蹤（不讀取或寫入統計檔案；現有資料將保留）
...
- `gac stats show` — 檢視你的 gac 使用統計（總計、連續使用、每日和每週活動、熱門專案）
- `gac stats project` — 僅檢視當前 git 專案的統計
...
gac 追蹤輕量級使用統計，讓你可以檢視提交活動、連續使用天數和最活躍的專案。統計資料本地儲存在 `~/.gac_stats.json` 中，不會傳送到任何地方。

**追蹤內容：** gac 執行總次數、提交總次數、首次/末次使用日期、每日和每週計數、當前和最長連續使用天數，以及每個專案的活動計數。

**不追蹤的內容：** 提交訊息、程式碼內容、檔案路徑、個人資訊，或任何超出計數和專案名稱的資料。
...
| `gac stats show`    | 顯示完整統計：總計、連續使用、每日和每週活動、熱門專案 |
| `gac stats project` | 僅顯示當前 git 專案的統計                              |
| `gac stats reset`   | 重置所有統計資料為零（需要確認）                       |
...
- **活動摘要** — 今日和本週的 gac 使用/提交數與你的峰值日和峰值週對比
- **熱門專案** — 按 gac 使用 + 提交數排列的 5 個最活躍倉庫
- **高分慶祝** — 🏆 在你創造新的每日、每週或連續使用紀錄時獲得獎盃；🥈 在追平紀錄時獲得
- **鼓勵訊息** — 基於你活動的上下文鼓勵

### 停用統計

將 `GAC_DISABLE_STATS` 環境變數設定為任意值：
...
停用後，gac 將跳過所有統計記錄 — 不會讀取或寫入檔案。現有資料將保留，但在取消設定該變數之前不會更新。
