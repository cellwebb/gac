...
- `gac auth qwen login` — Login to Qwen using OAuth device flow (opens browser)
- `gac auth qwen logout` — Logout from Qwen and remove stored token
- `gac auth qwen status` — Check Qwen authentication status
